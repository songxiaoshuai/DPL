...
class ClusterMetrics(Metric):
    def __init__(self, num_heads):
        ...
        self.add_state("feats", default=[])

    def update(self, preds: torch.Tensor, targets: torch.Tensor, feats=None):
    ...
    def compute(self):
        ...
        # print(self.feats)
        if self.feats != []:
            x_feats = torch.cat(self.feats, dim=0)
        targets -= targets.min()
        acc, nmi, ari, f1, precision, recall, score = [], [], [], [], [], [], []
        for head in range(self.num_heads):
            ...
            if self.feats != []:
                score.append(torch.tensor(silhouette_score(x, p), device=preds.device))

        return {"acc": acc, "nmi": nmi, "ari": ari, "f1": f1, "pre": precision, "rec": recall, "SC": score}
